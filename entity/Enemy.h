...
class Enemy : public EquippedEntity {
  ...
  public:
    Enemy(const std::string& name, double hp, double attk, double percDef, double staticDef, int64_t spd, std::array<std::string, 5> sprite = {
            "     ", "     ", "     ", "     ", "     "}, std::vector<std::shared_ptr<Item>> extraLoot = {}, std::function<Inventory(void)> invFactory = [](){ return Inventory(); }, std::function<std::shared_ptr<Weapon>(void)> weaponFactory = [](){ return IDefs::nothing; }, std::function<ArmorSet(void)> armorFactory = [](){ return ITiers::noArmor; });
